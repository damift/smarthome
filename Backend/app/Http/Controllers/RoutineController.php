...
class RoutineController extends Controller
{
    public function store(Request $request)
    {
        $validated = $request->validate([
            'slug' => ['required', 'string', 'max:100', 'alpha_dash', Rule::unique('routines', 'slug')],
            ...
            'steps.*.summary' => ['nullable', 'string', 'max:255'],
            'steps.*.room_name' => ['nullable', 'string', 'max:255'],
            'steps.*.type_name' => ['nullable', 'string', 'max:255'],
            'steps.*.device_name' => ['nullable', 'string', 'max:255'],
            'steps.*.action_name' => ['nullable', 'string', 'max:100'],
            'steps.*.action' => ['nullable', 'string', 'max:100'],
            'steps.*.value' => ['nullable'],
        ]);

        $steps = $this->normalizeSteps($validated['steps'] ?? []);
        if (empty($steps)) {
            return response()->json([
                'message' => 'A routine requires at least one valid step with an action name',
            ], 422);
        }
        ...
        return response()->json($this->normalizeRoutine($routine), 201);
    }

    public function update(Request $request, $routine)
    {
        ...
        $validated = $request->validate([
            'slug' => ['sometimes', 'string', 'max:100', 'alpha_dash', Rule::unique('routines', 'slug')->ignore($routineModel->id)],
        ...
        if (array_key_exists('slug', $validated)) {
            $routineModel->slug = strtolower(trim((string) $validated['slug']));
        }
        if (array_key_exists('title', $validated)) {
            $routineModel->title = $validated['title'];
        }
        if (array_key_exists('description', $validated)) {
            $routineModel->description = $validated['description'];
        }
        if (array_key_exists('icon', $validated)) {
            $routineModel->icon = $validated['icon'];
        }
        if (array_key_exists('is_active', $validated)) {
            $routineModel->is_active = (bool) $validated['is_active'];
        }
        if (array_key_exists('steps', $validated)) {
            ...
            $routineModel->steps = $steps;
        }

        $routineModel->save();
        $routineModel->refresh();

        return response()->json($this->normalizeRoutine($routineModel));
    }
